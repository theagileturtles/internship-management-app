...
export default async function handler(req, res) {
  ...
  try {
    ...
    let sql = "SELECT BIN_TO_UUID(letter_requests.uuid) AS UUID,image, BIN_TO_UUID(users.uuid) AS userUUID, users.first_name AS firstName, " +
      "users.last_name AS lastName, company,school_uuid AS studentID, status, letter_requests.created_at AS createdAt, letter_requests.updated_at, message, type, incomplete_internship AS incompleteInternships " +
